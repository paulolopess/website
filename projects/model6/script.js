document.addEventListener('DOMContentLoaded', () => {
  ...
  // TROCA AS TEXTURAS MUDANDO PARA O MODEL 2 ---------------------------------------------------------------------------------------
const changeTextureButton = document.querySelector('#change-texture-button');
const modelViewer = document.querySelector('model-viewer'); // Assuming you're using <model-viewer> for the GLTF model

if (changeTextureButton && modelViewer) {
  changeTextureButton.addEventListener('click', () => {
    console.log('Texture button clicked!');

    // Define the paths for the two models
    const modelOriginal = 'model6.gltf'; // Original model with default textures
    const modelWithNewTextures = './swichtextures/model6_2.gltf'; // Model with updated textures (textures2)

    // Check which model is currently being shown and switch to the other one
    const currentModel = modelViewer.getAttribute('src');
    
    if (currentModel === modelOriginal) {
      // Switch to the model with new textures
      modelViewer.setAttribute('src', modelWithNewTextures);
      console.log('Switched to the model with new textures.');
    } else {
      // Switch back to the original model
      modelViewer.setAttribute('src', modelOriginal);
      console.log('Switched back to the original model.');
    }
  });
}
